...
import { sendMsg } from '../realtime/ws-client.js';
  ...
  state.recognition.onresult = (e) => {
    ...
    for (let i = e.resultIndex; i < e.results.length; i++) {
      ...
      if (e.results[i].isFinal) {
        ...
        // Stream each finalized chunk to the backend so the realtime actor's
        // answer text matches what we display.
        if (state.session.socket && state.session.currentOrdinal != null) {
          sendMsg(state.session.socket, {
            t: 'utterance',
            seq: ++state.session.utteranceSeq,
            ordinal: state.session.currentOrdinal,
            text: text.trim(),
            is_final: true,
          });
        }
